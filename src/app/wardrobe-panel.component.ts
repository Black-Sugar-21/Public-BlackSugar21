...
const VENUES = ['cafe', 'restaurant', 'intimate_dinner', 'bar', 'night_club', 'cinema', 'theater', 'museum', 'park', 'viewpoint', 'hiking', 'bowling_alley', 'escape_room', 'bookstore', 'formal', 'celebration', 'trip', 'at_home'] as const;
const VENUE_EMOJI: Record<string, string> = { cafe: '☕', restaurant: '🍽️', intimate_dinner: '🕯️', bar: '🍸', night_club: '💃', cinema: '🎬', theater: '🎭', museum: '🖼️', park: '🌳', viewpoint: '🌇', hiking: '🥾', bowling_alley: '🎳', escape_room: '🗝️', bookstore: '📚', formal: '🤵', celebration: '🎉', trip: '✈️', at_home: '🏠' };
...
@Component({
  ...
  <div class="wd">
    ...
    @if (unclearPhoto()) {
      <div class="wd-unclear" role="alert">
        <span class="wd-unclear-ic">📷</span>
        <div class="wd-unclear-body">
          <b>{{ t('unclearTitle') }}</b>
          <p>{{ t('unclearMsg') }}</p>
          <label for="wdFile" class="wd-unclear-retry">{{ t('add') }}</label>
        </div>
        <button class="wd-del" (click)="unclearPhoto.set(false)" aria-label="✕">✕</button>
      </div>
    }
  ...
  styles: [`
    ...
    .wd-unclear { position: relative; display: flex; gap: 12px; align-items: flex-start; margin: 12px 0; padding: 14px; border-radius: 14px; background: rgba(212,175,55,.07); border: 1px solid rgba(212,175,55,.3); }
    .wd-unclear-ic { font-size: 26px; }
    .wd-unclear-body b { font-size: 14px; }
    .wd-unclear-body p { margin: 4px 0 10px; font-size: 13px; color: #C9C9D4; }
    .wd-unclear-retry { display: inline-block; background: linear-gradient(135deg,#E9CE6B,#D4AF37,#B8860B); color: #1A1206; padding: 8px 14px; border-radius: 10px; font-weight: 700; font-size: 13px; cursor: pointer; }
    .wd-unclear-retry:focus-visible { outline: 2px solid #D4AF37; outline-offset: 2px; }
...
export class WardrobePanelComponent implements OnInit, OnDestroy {
  ...
  // R153: no_garment_detected gets a dedicated actionable card (retake a clearer
    ...
    try {
      ...
      if (!res?.success) {
        if (res?.error === 'no_garment_detected') {
          ...
          const map: Record<string, string> = { wardrobe_full: 'full', rate_limit_exceeded: 'rateLimit' };
