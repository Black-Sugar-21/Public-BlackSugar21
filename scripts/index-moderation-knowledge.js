#!/usr/bin/env node
/**
 * RAG Indexing Script for Moderation Knowledge Base
 *
 * Reads moderation rules from JSON, generates embeddings with gemini-embedding-001,
 * and stores them in Firestore `moderationKnowledge` collection for native vector search.
 *
 * Same architecture as index-coach-knowledge.js but for the moderation pipeline.
 *
 * Usage:
 *   node scripts/index-moderation-knowledge.js                 # Index all
 *   node scripts/index-moderation-knowledge.js --clean         # Delete existing + reindex
 *   node scripts/index-moderation-knowledge.js --dry-run       # Preview without writing
 *
 * Prerequisites:
...
// Resolve @google/generative-ai from functions/node_modules
const {GoogleGenerativeAI} = require(path.join(__dirname, '..', 'functions', 'node_modules', '@google', 'generative-ai'));

// ─── Config ──────────────────────────────────────────────────────────────────
const COLLECTION = 'moderationKnowledge';
const EMBEDDING_MODEL = 'gemini-embedding-001';
const EMBEDDING_DIMENSIONS = 768;
const BATCH_SIZE = 10;
const EMBEDDING_DELAY_MS = 6500; // Rate limit: 100 req/min = 6s per batch of 10
...
function loadKnowledgeBase() {
  const knowledgeDir = path.join(__dirname, 'moderation-knowledge');
...
async function indexToFirestore(chunks, dryRun = false) {
  console.log(`\n📊 Indexing ${chunks.length} moderation knowledge chunks...`);
  let indexed = 0;
  let failed = 0;
...
async function main() {
  ...
  console.log('🛡️  Moderation — RAG Knowledge Base Indexer');
  ...
  console.log(`📚 Loaded ${chunks.length} moderation knowledge chunks`);
  ...
  if (!dryRun) {
    ...
    console.log(`   2. Deploy functions: firebase deploy --only functions --force`);
  }
 ...
 *   --collection-group=moderationKnowledge \
 *   --query-scope=COLLECTION \
 *   --field-config='vector-config={"dimension":"768","flat":"{}"},field-path=embedding'
 *
 * This creates a flat COSINE vector index on the `embedding` field.
 * Takes a few minutes to build. Check status:
 *   gcloud firestore indexes composite list --project=black-sugar21
 */
